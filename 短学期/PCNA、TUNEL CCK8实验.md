...
## 一、 CCK-8 检测细胞活性
### 1. 实验原理
CCK-8（Cell Counting Kit-8）试剂中含有 WST-8。在电子载体 1-Methoxy PMS 的作用下，活细胞线粒体内的脱氢酶可将 WST-8 还原为高度水溶性的黄色甲臜（Formazan）染料。生成的甲臜数量与活细胞数量成正比。通过酶标仪测定 450 nm 波长处的吸光度（OD值），即可间接反映活细胞的数量，用于评估细胞增殖或药物的细胞毒性。
### 2. 操作步骤
1. 96孔板中的细胞，100ul培养液；
...
- **药物自身吸光度干扰**：若加入的药物本身具有颜色或能还原 WST-8。必须设置“药物+CCK8+无细胞”的本底对照孔，在最终计算时扣除。
## 二、 PCNA 免疫荧光
### 1. 实验原理
